...
如果控制器内有多个 ``view()`` 调用,CodeIgniter 将智能地处理它们。如果发生多次调用,它们将被追加在一起。例如,你可能希望有一个头部视图、菜单视图、内容视图和页脚视图。可能如下所示:
...
你可以通过在第三个参数中传递 ``cache`` 选项和缓存视图的秒数来缓存 ``view()`` 函数的视图:
...
通过 ``view()`` 函数的第二个参数中的数组,控制器的数据传递给视图。这里有个例子:
...
传入的数据在对 ``view()`` 的后续调用中保留。如果在单个请求中多次调用该函数,则不必为每个 ``view()`` 传递所需的数据。
