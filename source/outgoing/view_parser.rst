...
你可以通过编辑 **app/Config/View.php** 并向 ``$filters`` 数组添加新条目来轻松创建自己的过滤器。每个键是在视图中调用过滤器的名称，其值是任何有效的 PHP 可调用函数：
...
你可以通过编辑 **app/Config/View.php** 并向 ``$filters`` 数组添加新条目，将原生的 PHP 函数用作过滤器。每个键是在视图中调用的原生 PHP 函数的名称，其值是任何有效的原生 PHP 函数，前缀为：
