...
实体可包含隐藏属性，此类属性名以下划线（``_``）开头。默认情况下，调用 ``toArray()`` 时不会包含这些隐藏属性。如有需要，仍可直接访问。

若需在 ``toArray()`` 的输出结果中包含隐藏属性，则需使用 ``datamap`` 功能使之“可见”。

请注意，（下一节将介绍的）智能 ``__get()`` 与 ``__set()`` 方法会忽略属性名的前导下划线。这意味着同名的两个属性（一个带 ``_`` 前缀，另一个不带）会共用相同的 Getter 和 Setter，因此必须明确默认处理哪一个属性。
...
属性可转换为以下任意数据类型：
**integer** 、**float** 、**double** 、**string** 、**boolean** 、**object** 、**array** 、**datetime** 、**timestamp** 、**uri** 、**int-bool** 以及 **enum**。
在类型开头添加问号可将属性标记为可空，例如 **?string** 、**?integer**。

.. note:: **int-bool** 自 v4.3.0 起可用。
.. note:: **enum** 自 v4.7.0 起可用。

例如，若 User 实体具有 ``is_banned`` 属性，可将其转换为布尔值：
...
可将属性转换为 PHP Enum。转换时必须将 Enum 类名指定为参数。
...
例如，若 User 实体的 ``status`` 属性使用了 Backed Enum：
...
此时访问 ``status`` 属性，该值将自动转换为 ``UserStatus`` Enum 实例：
...
实体类会对对象和数组执行 **深度比较** ，从而准确检测其内部状态的变更。

标量值
-------------

对于标量值（字符串、整型、浮点型、布尔值、null），实体使用直接比较。当实体中的所有属性均为标量时，将采用优化后的比较方式以提升性能。

对象与数组
------------------

针对对象和数组，实体会通过 JSON 编码和规范化处理后进行比较。这意味着对嵌套结构、对象属性、数组元素、嵌套实体（使用 ``toRawArray()`` ）、Enum（``BackedEnum`` 与 ``UnitEnum``）、DateTime 对象（``DateTimeInterface``）、集合（``Traversable``）、具有 ``__toString()`` 的值对象，以及实现了 ``JsonSerializable`` 或 ``toArray()`` 的对象所做的修改，均能被准确检测。
