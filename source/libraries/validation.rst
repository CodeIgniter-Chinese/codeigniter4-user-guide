...
规则                    参数       描述                                          示例
======================= ========== ============================================= ===================================================
alpha                   无         若字段包含 ASCII 字母以外的内容，则校验失败。
alpha_dash              无         若字段包含 ASCII 字母数字、
                                   下划线或减号以外的内容，则校验失败。
alpha_numeric           无         若字段包含 ASCII 字母数字以外的内容，
                                   则校验失败。
alpha_numeric_punct     无         若字段包含字母数字、空格或以下指定标点符号
                                   以外的内容，则校验失败：
                                   ``~``（波浪号）、``!``（感叹号）、
                                   ``#``（井号）、``$``（美元符号）、
                                   ``%``（百分号）、``&``（与符号）、
                                   ...
                                   ``=``（等号）、``|``（竖线）、
                                   ``:``（冒号）、``.``（点号）。
alpha_numeric_space     无         若字段包含 ASCII 字母数字或空格以外的内容，
                                   则校验失败。
alpha_space             无         若字段包含 ASCII 字母或空格以外的内容，
                                   则校验失败。
