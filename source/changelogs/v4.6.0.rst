...
- 新增 ``Feature::$strictLocaleNegotiation`` 特性开关，用于启用严格的语言环境匹配。
  此前，若请求头包含 ``Accept-language: en-US,en-GB;q=0.9``，系统可能会返回首个匹配的语言（如 ``en``），而非精确匹配 ``en-US`` 或 ``en-GB``。
  将此值设为 ``true`` 后，匹配逻辑将不再仅基于语言代码（'en' - ISO 639-1），还会校验地区代码（'en-US' - ISO 639-1 及 ISO 3166-1 alpha）。
