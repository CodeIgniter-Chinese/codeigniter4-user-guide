...
.. important:: 为了初始化 Forge 类,您的数据库驱动程序必须已经运行,因为 Forge 类依赖于它。

如下加载 Forge 类:

.. literalinclude:: forge/001.php

您也可以向 DB Forge 加载器传递另一个数据库组名称,以防要管理的数据库不是默认数据库:
...
允许您创建第一个参数中指定的数据库。基于成功或失败返回 true/false:
...
允许您删除第一个参数中指定的数据库。基于成功或失败返回 true/false:
...
如果一切顺利,您应该会看到显示的 ``Database "foo" successfully created.`` 消息。

如果您在测试环境中或正在使用 SQLite3 驱动程序,可以使用 ``--ext`` 选项
...
在创建表时,您可能希望执行几件事。添加字段、向表添加键、更改列。CodeIgniter 为此提供了一种机制。
...
字段通常通过关联数组创建。在数组中,您必须包含与字段的数据类型相关的 ``type`` 键。 例如,INT、VARCHAR、TEXT 等。许多数据类型(例如 VARCHAR)还需要一个 ``constraint`` 键。
...
通常,您会希望表具有键。这是通过 ``$forge->addKey('field')`` 完成的。可选的第二个参数设置为 true 将使其成为主键,第三个参数设置为 true 将使其成为唯一键。您可以使用第四个参数指定名称。请注意,``addKey()`` 必须在表已存在的情况下后跟对 ``createTable()`` 或 ``processIndexes()`` 的调用。
...
.. note:: 当您添加主键时,即使提供了名称,MySQL 和 SQLite 也会假定名称为 ``PRIMARY``。
...
您还可以指定约束的“更新时”和“删除时”属性的所需操作以及名称:
...
您也可以传递可选的表属性,例如 MySQL 的 ``ENGINE``:
...
您可以通过使用 ``addKey()``、``addPrimaryKey()``、``addUniqueKey()`` 或 ``addForeignKey()`` 和 ``processIndexes()`` 向现有表添加键:
