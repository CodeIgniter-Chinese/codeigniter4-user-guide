...
您可以使用 ``resource()`` 方法快速为单个资源创建一小 handful 的 RESTful 路由。这会创建完全 CRUD 资源所需的最常见的 5 个路由:创建新资源、更新现有资源、列出所有该资源、显示单个资源以及删除单个资源。第一个参数是资源名称:

.. literalinclude:: restful/001.php

.. note:: 上面的顺序是为了清晰起见,而实际创建路由的顺序在 RouteCollection 中确保了正确的路由解析

.. important:: 路由按指定顺序匹配,因此如果您在上方有一个资源 photos,然后有一个 get 'photos/poll',资源线的 show 操作的路由将在 get 线之前匹配。要解决此问题,请将 get 行移动到资源行之上,以便先匹配它。

第二个参数接受可以用于修改生成的路由的选项数组。虽然这些路由面向 API 使用,其中允许更多方法,但您可以传入 ``websafe`` 选项,使其生成适用于 HTML 表单的 update 和 delete 方法:
...
您可以通过使用应该使用的控制器的名称传递 ``controller`` 选项来指定应该使用的控制器:
...
默认情况下,当需要资源 ID 时,会使用 ``(:segment)`` 占位符。您可以通过传递 ``placeholder`` 选项及要使用的新字符串来更改此占位符:
...
您可以使用 ``only`` 选项限制生成的路由。这应该是 **数组** 或 **以逗号分隔的方法名列表**,应该创建这些方法。仅将创建与这些方法之一匹配的路由。其余的会被忽略:

.. literalinclude:: restful/005.php

否则,您可以使用 ``except`` 选项删除未使用的路由。这也应该是 **数组** 或 **以逗号分隔的方法名列表**。此选项在 ``only`` 之后运行:
...
``ResourceController`` 为您的 RESTful API 提供了一个方便的起点,其方法对应于上面的资源路由。

扩展它,覆盖 ``modelName`` 和 ``format`` 属性,然后实现您想要处理的那些方法:
...
您可以使用 ``presenter()`` 方法快速创建与资源控制器对齐的表示控制器。
这将创建对应于上面的资源控制器方法的路由,这些方法会为您的资源返回视图,
或处理来自这些视图的表单提交。
...
扩展它,重写 ``modelName`` 属性,然后实现您想要处理的方法:
