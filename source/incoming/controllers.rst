...
控制器就是一个处理 HTTP 请求的类文件。:doc:`URI 路由 <routing>` 将 URI 与控制器关联起来。它返回视图字符串或 ``Response`` 对象。
...
.. important:: 不能在构造函数中使用 ``return``。所以 ``return redirect()->to('route');`` 不会起作用。
...
应用程序的主 :doc:`Request 实例 </incoming/incomingrequest>` 始终作为类属性 ``$this->request`` 可用。

Response 对象
===============

应用程序的主 :doc:`Response 实例 </outgoing/response>` 始终作为类属性 ``$this->response`` 可用。

Logger 对象
=============

:doc:`Logger <../general/logging>` 类的实例作为类属性 ``$this->logger`` 可用。
...
.. note:: 有许多 :doc:`基于时间的常量 </general/common_functions>` 始终可供使用，包括 ``YEAR``、``MONTH`` 等。
...
:doc:`验证库文档 </libraries/validation>` 详细说明了规则和消息数组格式以及可用的规则：
...
.. important:: 此方法仅为向后兼容而存在。不要在新项目中使用它。即使已经在使用，我们也建议使用 ``validateData()`` 方法代替。

控制器还提供了便捷方法 ``validate()``。

.. warning:: 应使用 ``validateData()`` 而不是 ``validate()`` 来仅验证 POST 数据。``validate()`` 使用 ``$request->getVar()`` 返回 ``$_GET``、``$_POST`` 或 ``$_COOKIE`` 数据（按该顺序，取决于 php.ini `request-order <https://www.php.net/manual/zh/ini.core.php#ini.request-order>`_）。较新的值会覆盖较旧的值。如果 Cookie 具有相同的名称，POST 值可能会被覆盖。
...
.. warning:: 当使用 ``validate()`` 方法时，应该使用 :ref:`getValidated() <validation-getting-validated-data>` 方法来获取验证后的数据。因为 ``validate()`` 方法在内部使用 :ref:`Validation::withRequest() <validation-withrequest>` 方法，它验证来自 :ref:`$request->getJSON() <incomingrequest-getting-json-data>` 或 :ref:`$request->getRawInput() <incomingrequest-retrieving-raw-data>` 或 :ref:`$request->getVar() <incomingrequest-getting-data>` 的数据，攻击者可能会更改验证的数据。
...
.. note:: 验证也可以在模型中自动处理，但有时在控制器中更容易。具体位置由你决定。
...
.. important:: 此功能仅为向后兼容而存在。不要在新项目中使用它。即使已经在使用，我们也建议使用 :ref:`auto-routing-improved` 代替。

本节描述了自动路由（传统版）的功能，这是来自 CodeIgniter 3 的路由系统。它自动路由 HTTP 请求，并在没有路由定义的情况下执行相应的控制器方法。自动路由默认是禁用的。

.. warning:: 为了防止错误配置和错误编码，我们建议不要使用自动路由（传统版）。很容易创建易受攻击的应用程序，其中控制器过滤器或 CSRF 保护被绕过。

.. important:: 自动路由（传统版）使用 **任何** HTTP 方法将 HTTP 请求路由到控制器方法。

.. important:: 从 v4.5.0 开始，如果自动路由（传统版）找不到控制器，它将在控制器过滤器执行之前抛出 ``PageNotFoundException`` 异常。
...
.. note:: 当控制器的短名称与 URI 的第一分段匹配时，它将被加载。
...
.. important:: 文件必须命名为 **Helloworld.php**，首字母 ``H`` 大写。使用自动路由时，控制器类名必须以大写字母开头，且只有第一个字符可以大写。
...
.. note::
    ...
    如果想要另一种命名约定，需要使用 :ref:`定义路由 <defined-route-routing>` 手动定义它。以下是基于 PSR-4 自动加载器的示例：
...
.. note:: ``$routes->get('/', 'Home::index');`` 这行代码是一个优化，你会希望在“真实世界”的应用程序中使用它。但出于演示目的，我们不想使用该功能。``$routes->get()`` 在 :doc:`URI 路由 <routing>` 中有说明。
...
.. important:: 目录名必须以大写字母开头，且只有第一个字符可以大写。
...
.. note:: 不能在 **app/Controllers** 和 **public/** 中使用相同名称的目录。这是因为如果存在目录，Web 服务器将搜索它，并且不会路由到 CodeIgniter。
...
CodeIgniter 还支持使用 :ref:`定义路由 <defined-route-routing>` 映射 URI。
...
.. important:: 如果控制器包含名为 ``_remap()`` 的方法，无论 URI 包含什么，它都将 **始终** 被调用。它覆盖了 URI 确定调用哪个方法的正常行为，允许定义自己的方法路由规则。
...
如果要扩展控制器，请参阅 :doc:`../extending/basecontroller`。
