...
CodeIgniter 提供了两种独立的工具来帮助你基准测试代码和测试不同选项:
**Timer** 和 **Iterator**。Timer 允许你轻松计算脚本执行过程中的两点之间的时间。Iterator 允许你设置几种变体并运行这些测试,记录性能和内存统计数据以帮助你决定哪种版本最佳。
...
或者,你可以使用 :doc:`全局函数 </general/common_functions>` ``timer()`` 来启动和停止计时器:
